--- html5_notifier.php.orig	2023-11-05 13:34:20.055266000 +0200
+++ html5_notifier.php	2023-11-05 13:34:39.959914000 +0200
@@ -1,146 +1,144 @@
-<?php
-/**
- * html5_notifier
...
+        $this->add_hook('preferences_save', array($this, 'prefs_save'));
+
+        if ($RCMAIL->config->get('html5_notifier_duration').'') {
+            $this->add_hook('new_messages', array($this, 'show_notification'));
+        }
+
+        $this->include_script("html5_notifier.js");
+
+        if ($RCMAIL->action != 'check-recent') {
+            $this->add_texts('localization', array('notification_title', 'ok_notifications', 'no_notifications', 'check_ok', 'check_fail', 'check_fail_blocked'));
+        }
+    }
+
+    public function show_notification($args)
+    {
+        $RCMAIL = \rcmail::get_instance();
+
+        $deleted = $RCMAIL->config->get('skip_deleted') ? 'UNDELETED ' : '';
...
+            switch ($RCMAIL->config->get('html5_notifier_smbox')) {
+                case 1:
+                    $mbox = array_pop(explode('.', str_replace('INBOX.', '', $args['mailbox'])));
+                    break;
+                case 2:
